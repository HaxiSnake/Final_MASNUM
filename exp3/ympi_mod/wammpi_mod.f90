...
! Modified by Zhenya.Song, for point check, 2016/04/06
  integer, parameter :: ipoint = 200!225
  integer, parameter :: jpoint = 300!50
! End modify
...
!    dtime = dtime0 + key * (it - number) * delttm / 1440.
    ...
    if(myid==0)then
        if(dtime > dtimeend)then
            ...
            write(11,*)"Propagat Time is ",ptime," seconds."
            write(11,*)"Implsch Time is ",imtime," seconds."
            close(11)
        endif
    endif
...
!        write(*,*)"id=",myid,"The propagat time:",(count_end-count_start)*1.0/count_rate,"(s)."
        ptime=ptime+(count_end-count_start)*1.0/count_rate
    else
!        write(*,*)"id=",myid,"The propagat time:",(count_end-count_start+count_max)*1.0/count_rate,"(s)."
        ptime=ptime+(count_end-count_start+count_max)*1.0/count_rate
    endif
!    endif
...
!    if(myid==0)then
    if (count_end > count_start)then
!        write(*,*)"id=",myid,"The propagat time:",(count_end-count_start)*1.0/count_rate,"(s)."
        imtime=imtime+(count_end-count_start)*1.0/count_rate
    else
!        write(*,*)"id=",myid,"The propagat time:",(count_end-count_start+count_max)*1.0/count_rate,"(s)."
        imtime=imtime+(count_end-count_start+count_max)*1.0/count_rate
    endif
!    endif
